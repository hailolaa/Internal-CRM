...
function serializeGrowthScoreCategories(categories: NormalizedContactData["growthScoreCategories"]) {
  const hasValue = Object.values(categories).some((value) => value !== null && value !== undefined);
  return hasValue ? JSON.stringify(categories) : null;
}
...
export async function insertContact(
  ...
  const columns = [
    ...
    "website", "date_of_birth", "gender", "address", "city", "state", "postal_code", "country", "tags", "status", "lead_status",
    "lost_reason", "objection_type", "source", "value",
    ...
    "treatment_interests", "package_interest", "recommended_package",
  ...
  const values = [
    ...
    serializeGrowthScoreCategories(contact.growthScoreCategories),
    contact.growthScoreCategories.websiteVisibility,
    contact.growthScoreCategories.seo,
    contact.growthScoreCategories.gbp,
    contact.growthScoreCategories.tracking,
    contact.growthScoreCategories.conversion,
    contact.growthScoreCategories.leadHandling,
    contact.growthScoreCategories.responseSpeed,
    contact.growthScoreCategories.enquiryVisibility,
    contact.growthScoreCategories.treatmentPerformance,
    contact.growthScoreCategories.revenueLeakage,
    contact.growthScoreCategories.growthOpportunity,
...
export async function insertImportedContact(
  ...
  const values = [
    ...
    row.growthScoreOverall,
    serializeGrowthScoreCategories(row.growthScoreCategories),
    row.growthScoreCategories.websiteVisibility,
    row.growthScoreCategories.seo,
    row.growthScoreCategories.gbp,
    row.growthScoreCategories.tracking,
    row.growthScoreCategories.conversion,
    row.growthScoreCategories.leadHandling,
    row.growthScoreCategories.responseSpeed,
    row.growthScoreCategories.enquiryVisibility,
    row.growthScoreCategories.treatmentPerformance,
    row.growthScoreCategories.revenueLeakage,
    row.growthScoreCategories.growthOpportunity,
  ...
  await pool.execute(
    `UPDATE contact
     SET account_name = COALESCE(account_name, ?),
         ...
         growth_score_overall = COALESCE(?, growth_score_overall),
         growth_score_categories = COALESCE(?, growth_score_categories),
         growth_score_website_visibility = COALESCE(?, growth_score_website_visibility),
         growth_score_seo = COALESCE(?, growth_score_seo),
         growth_score_gbp = COALESCE(?, growth_score_gbp),
         growth_score_tracking = COALESCE(?, growth_score_tracking),
         growth_score_conversion = COALESCE(?, growth_score_conversion),
         growth_score_lead_handling = COALESCE(?, growth_score_lead_handling),
         growth_score_response_speed = COALESCE(?, growth_score_response_speed),
         growth_score_enquiry_visibility = COALESCE(?, growth_score_enquiry_visibility),
         growth_score_treatment_performance = COALESCE(?, growth_score_treatment_performance),
         growth_score_revenue_leakage = COALESCE(?, growth_score_revenue_leakage),
         growth_score_growth_opportunity = COALESCE(?, growth_score_growth_opportunity),
