...
import { ProposalV5MobileRenderer, isProposalV5Snapshot, type ProposalV5Snapshot } from "@/components/proposals/v5";
import {
...
const previewRoute = "/app/crm/proposals/v5-mobile-preview";
...
export function ProposalV5PrivateMobilePreviewView({
  clinicTypeInput,
  packageIdInput,
  longContent,
  persistedSnapshot,
  ...
  return (
    <div className="proposal-v5-private-mobile-preview">
      <section
        aria-label="Private V5 mobile preview controls"
        className="mb-6 rounded-[8px] border border-[#c8dfdd] bg-[#f4fafa] p-5 text-[#132e35]"
      >
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#2f9e99]">Private internal preview</p>
            <h1 className="mt-2 text-2xl font-semibold text-[#011418]">ClinicGrower V5 mobile proposal renderer</h1>
            <p className="mt-2 text-sm leading-6 text-[#5f777b]">
              {isPersistedPreview
                ? `Rendering the frozen V5 snapshot stored on ${persistedProposalName || "this proposal"} through the isolated mobile renderer.`
                : "This route renders the isolated mobile V5 renderer through the real Next app. It is not wired into public proposal links, the editor, acceptance or customer navigation."}
            </p>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Link
            href={`/app/crm/proposals/v5-preview?clinicType=${encodeURIComponent(clinicType)}&package=${encodeURIComponent(snapshot.selectedPackage.id || packageId)}${longContent ? "&case=long" : ""}`}
            className="rounded-[6px] border border-[#c8dfdd] bg-white px-3 py-2 text-xs font-semibold text-[#132e35] hover:border-[#2f9e99]"
          >
            Open desktop V5 preview
          </Link>
        </div>
      </section>
...
function ProposalV5PersistedSnapshotMobilePreview({
  proposalId,
  token,
}: {
  proposalId: string;
      ...
      .then((proposal) => {
        ...
        if (!isProposalV5Snapshot(proposal.v5Snapshot)) {
          setState({
            ...
            message: "This proposal does not have a frozen V5 snapshot yet. Send the V5 proposal first, then reopen this mobile preview.",
          });
          return;
        }
        setState({
  ...
  return (
    <ProposalV5PrivateMobilePreviewView
...
export function ProposalV5PrivateMobilePreviewContent() {
  ...
  if (proposalId && !token) {
    return (
      <section className="rounded-[8px] border border-[#c8dfdd] bg-[#f4fafa] p-5 text-sm text-[#132e35]">
        Sign in before opening a frozen proposal snapshot mobile preview.
      </section>
    );
  }

  if (proposalId && token) {
    return <ProposalV5PersistedSnapshotMobilePreview key={proposalId} proposalId={proposalId} token={token} />;
  }
